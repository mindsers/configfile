    ...
    inquirer.prompt(questions)
      .then(data => {
        ...
        if (!fileExist(folder_path)) {
          log({ type: 'info', message: 'Folder does not exist. It will be created.', prefix: ' Info '})
      ...
      .then(({ repo_name, folder_path }) => {
        if (repo_name == null) {
          log({ type: 'warn', message: 'Impossible to cloned git repository. Need repository URL.', prefix: ' Warn ' })
          return { repo_name, folder_path }
        }
        
        return gitClone(repo_name, folder_path)
          .then((repo) => {
            log({ type: 'info', message: 'Git repository cloned successuly.', prefix: ' Info ' })
            return { repo_name, folder_path }
          })
      })
      ...
      .catch((error) => {
        switch (error.message) {
          case 'folder_is_not_directory':
            log({ type: 'error', message: `${folder_path} is not a folder.`, prefix: ' Abort ' })
            break
          case 'folder_is_not_empty':
            log({ type: 'error', message: 'Folder is not empty.', prefix: ' Abort ' })
            break
          case 'user_stop_init':
